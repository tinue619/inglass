...
// Middleware
app.use(cors({
    origin: '*', // Для разработки. В продакшене указать конкретные домены
...
async function startServer() {
    try {
        ...
        // Запускаем сервер
        app.listen(PORT, () => {
            console.log(`🚀 Inglass CRM Server с SQLite запущен на порту ${PORT}`);
            ...
            console.log('  POST /api/data       - синхронизация данных');
            console.log('  POST /api/auth/login - аутентификация');
            console.log('  CRUD /api/users      - управление пользователями');
            console.log('  CRUD /api/processes  - управление процессами');
            console.log('  CRUD /api/products   - управление изделиями');
            console.log('  CRUD /api/orders     - управление заказами');
            console.log('');
        });
